(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('dPaaSAuthApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-dPaaSAuthApp-error');
                var entityKey = httpResponse.headers('X-dPaaSAuthApp-params');
                if (errorHeader) {
